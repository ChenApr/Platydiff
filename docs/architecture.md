...
perceptual relations, including audio-only schema v6. Accepted
[RFC 0011](rfcs/0011-rfc-0009-audio-preflight-amendment.md) freezes the P7-A1
preflight wire contracts, WAV/PCM profile, CLI/plugin rejection, and stable
failure details. Accepted
[RFC 0012](rfcs/0012-p7a1-audio-pre-code-contract-closure.md) closes equality
carriers, change grouping/counts, resource limits, encoded-byte bypass
behavior, timing facts, and problem details. Audio remains unimplemented; RFC
0010 records conditional authorization for P7-A1 dispatch only after the
actual schema-v3, schema-v4, schema-v5, and compatibility-fixture predecessors
merge to `main`, and RFC 0011/0012 acceptance does not start implementation.
...
audio only, while accepted [RFC 0011](rfcs/0011-rfc-0009-audio-preflight-amendment.md)
and [RFC 0012](rfcs/0012-p7a1-audio-pre-code-contract-closure.md) close the
P7-A1 pre-code audio contract details. Video waits for a later backend/worker
amendment and the next successor. They are not implemented, and public schema
merges must respect predecessor order even when design and backend research
