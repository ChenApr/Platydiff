...
audio-only schema v6。Accepted
[RFC 0011](rfcs/0011-rfc-0009-audio-preflight-amendment_zh.md) 冻结 P7-A1 preflight
wire contract、WAV/PCM profile、CLI/plugin rejection 与 stable failure detail。Accepted
[RFC 0012](rfcs/0012-p7a1-audio-pre-code-contract-closure_zh.md) 闭合 equality carrier、
change grouping/count、resource limit、encoded-byte bypass behavior、timing fact 与
problem detail。Audio 仍未实现；RFC 0010 记录 P7-A1 的条件授权，但 coordinator 只有在
实际 schema-v3、schema-v4、schema-v5 与 compatibility fixture 前驱都合并到 `main` 后
才能派发，且 RFC 0011/0012 acceptance 不会启动实现。
...
[RFC 0009](rfcs/0009-audio-and-video-comparison_zh.md) 也把 video 的显式契约接受为 roadmap
direction：stream structure、decoded frame、frame metric、perceptual video 与 audio-track
association。Video 仍未实现，写代码前必须等待后续 backend/worker amendment 与下一个 schema
successor。
...
它只为 audio 接受 schema v6，Accepted
[RFC 0011](rfcs/0011-rfc-0009-audio-preflight-amendment_zh.md) 与
[RFC 0012](rfcs/0012-p7a1-audio-pre-code-contract-closure_zh.md) 闭合 P7-A1 pre-code
audio contract detail。Video 需等待后续 backend/worker amendment 与下一个 successor。
